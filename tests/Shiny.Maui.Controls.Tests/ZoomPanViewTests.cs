...
public class ZoomPanViewTests
{
    ...
    static bool Has<T>(View view) where T : IGestureRecognizer
        => view.GestureRecognizers.OfType<T>().Any();
    ...
    public async Task BelowNaturalSizeThereIsNothingToPan()
    {
        ...
        view.Scale.ShouldBe(0.5);
        view.TranslationX.ShouldBe(0);
        view.TranslationY.ShouldBe(0);
        view.IsZoomed.ShouldBeFalse();
        Has<PanGestureRecognizer>(view).ShouldBeFalse();
    }
    ...
    {
        new ZoomPanView().IsClippedToBounds.ShouldBeTrue();
    }
}
