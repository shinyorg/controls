...
- `src/` — one project per package per host. Core: `Shiny.Maui.Controls`, `Shiny.Blazor.Controls`. Add-ons on **both** hosts: `*.Barcodes`, `*.Camera`, `*.Camera.Ai`, `*.Diagram`, `*.FloorPlan`, `*.Gamepad`, `*.Markdown`, `*.MediaElement`, `*.MermaidDiagrams`, `*.Office`, `*.SpeechAddins`, themes `*.Themes.Aurora/Material/Ocean/Terminal`. **MAUI-only**: `Shiny.Maui.Controls.Desktop`, `Shiny.Maui.Controls.Keyframe` (+ `.Keyframe.Export`), `Shiny.Maui.Controls.MediaElement.Linux`, camera analyzers `Camera.Barcode/Documents/Face/Motion/Ocr`. Cross-host shared libraries (referenced by the hosts, also published): `Shiny.Controls.Camera.Shared`, `.Diagram.Shared` (the diagramming engine — the graph model, tidy tree / layered / mindmap / radial / force-directed layouts, connector routing and shape-outline geometry; dependency-free), `.Gamepad.Shared` (the on-screen gamepad engine — layouts, multi-touch hit testing, stick/d-pad maths, and the `VirtualGamepad`/`VirtualGamepadManager` that make it a Shiny.Gamepad `IGamepad`), `.FloorPlan.Shared` (the floor plan engine — document model, Skia renderers, hit-testing, camera, tools; the reason FloorPlan is an add-on and not core is that it drags SkiaSharp with it), `.Gantt.Shared` (the Gantt scheduling engine — calendars, dependency cascades, critical path, time-to-pixel geometry), `.Keyframe.Shared`, `.MediaElement.Shared`, `.MotionIcons.Shared`, `.Office.Shared`, `.Office.Skia`. There is **no** `*.Kiosk` package — the Blazor on-screen keyboard and docking ship in `Shiny.Blazor.Controls`.
